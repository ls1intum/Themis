...
class ProblemStatementCellViewModel: ObservableObject {
    ...
    @Published var problemStatementParts: [AnyType]

    init(problemStatement: String) async {
        self.problemStatementParts = await ProblemStatementCellViewModel.convertProblemStatement(problemStatement)
    }

    init(problemStatementParts: [AnyType]) {
        self.problemStatementParts = problemStatementParts
    }

    static func deleteTestCases(_ problemStatement: String) -> String {
    ...
    static func convertProblemStatement(_ problemStatement: String) async -> [AnyType] {
        let convertedString = deleteTestCases(problemStatement)

        var problemStatementParts: [AnyType] = []

        var currentPart = ""

        var currentlyUmlPart = false

        var endOfUmlIndex = -1

        for (index, char) in convertedString.enumerated() {
            if index == endOfUmlIndex {
            ...
            if char == "@" {
                if Array(convertedString)[index+1] == "s"
                    && Array(convertedString)[index+2] == "t"
                    && Array(convertedString)[index+3] == "a"
                    && Array(convertedString)[index+4] == "r"
                    && Array(convertedString)[index+5] == "t"
                    && Array(convertedString)[index+6] == "u"
                    && Array(convertedString)[index+7] == "m"
                    && Array(convertedString)[index+8] == "l" {
                    ...
                    currentPart = ""
                    currentlyUmlPart = true
                }
                if Array(convertedString)[index+1] == "e"
                    && Array(convertedString)[index+2] == "n"
                    && Array(convertedString)[index+3] == "d"
                    && Array(convertedString)[index+4] == "u"
                    && Array(convertedString)[index+5] == "m"
                    && Array(convertedString)[index+6] == "l" {
                    endOfUmlIndex = index + 7
                }
            }
    ...
    public static var mock: ProblemStatementCellViewModel {
        // let mock = ProblemStatementCellViewModel(problemStatement: mockData)

        let mock = ProblemStatementCellViewModel(problemStatementParts: [])
        mock.problemStatementParts.append(AnyType(part: deleteTestCases("# Sorting with the Strategy Pattern\n\nIn this exercise, we want to implement sorting algorithms and choose them based on runtime specific variables.\n\n### Part 1: Sorting\n\nFirst, we need to implement two sorting algorithms, in this case `MergeSort` and `BubbleSort`.\n\n**You have the following tasks:**\n\n1. [task][Implement Bubble Sort](testBubbleSort())\nImplement the method `performSort(List<Date>)` in the class `BubbleSort`. Make sure to follow the Bubble Sort algorithm exactly.\n\n2. [task][Implement Merge Sort](testMergeSort())\nImplement the method `performSort(List<Date>)` in the class `MergeSort`. Make sure to follow the Merge Sort algorithm exactly.\n\n### Part 2: Strategy Pattern\n\nWe want the application to apply different algorithms for sorting a `List` of `Date` objects.\nUse the strategy pattern to select the right sorting algorithm at runtime.\n\n**You have the following tasks:**\n\n1. [task][SortStrategy Interface](testClass[SortStrategy],testMethods[SortStrategy])\nCreate a `SortStrategy` interface and adjust the sorting algorithms so that they implement this interface.\n\n2. [task][Context Class](testAttributes[Context],testMethods[Context])\nCreate and implement a `Context` class following the below class diagram\n\n3. [task][Context Policy](testConstructors[Policy],testAttributes[Policy],testMethods[Policy])\nCreate and implement a `Policy` class following the below class diagram with a simple configuration mechanism:\n\n  1. [task][Select MergeSort](testClass[MergeSort],testUseMergeSortForBigList())\n  Select `MergeSort` when the List has more than 10 dates.\n\n  2. [task][Select BubbleSort](testClass[BubbleSort],testUseBubbleSortForSmallList())\n  Select `BubbleSort` when the List has less or equal 10 dates.\n\n4. Complete the `Client` class which demonstrates switching between two strategies at runtime.\n\n")))
        mock.problemStatementParts.append(AnyType(part: deleteTestCases("\n\n\n### Part 3: Optional Challenges\n\n(These are not tested)\n\n1. Create a new class `QuickSort` that implements `SortStrategy` and implement the Quick Sort algorithm.\n\n2. Make the method `performSort(List<Dates>)` generic, so that other objects can also be sorted by the same method.\n**Hint:** Have a look at Java Generics and the interface `Comparable`.\n\n3. Think about a useful decision in `Policy` when to use the new `QuickSort` algorithm.\n")))
        return mock
    }
